...
def index(request, user):
    ...
    return render(request, 'index.html', {'user': user, 'codes': cats, 'subcodes': subcats, 'cat_dict': cat_subcat, 'employees': employees, 'user_obj': user_obj})
    ...
    if request.POST['action'] == 'code':
        ...
        for entry in SubCat.objects.all():
            if entry.SubCategory == new_code:
                ...
                return redirect('timesheet:index', user=user)
